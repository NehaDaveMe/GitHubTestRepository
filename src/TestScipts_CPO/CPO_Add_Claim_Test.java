...
import ObjectRepository_CPO.CPO_Add_Claim;
...
public class CPO_Add_Claim_Test extends BaseClass
{
	...
	public void testClaims() throws Throwable {
		...
		js.executeScript("arguments[0].click();",CPO_Add_CP_EnrollmentPage.linkClaims(driver));
		Thread.sleep(1500);
		wb.getWindowIDs();
		...
		wb.waitForElemnet(CPO_Add_Claim.claimDate(driver));
		Thread.sleep(2000);
		CPO_Add_Claim.claimDate(driver).sendKeys("05/05/2020",Keys.TAB);
		test.log(LogStatus.PASS, " Claim Date Added Successfully");
		
		
		wb.selectSingleValue(CPO_Add_Claim.selectClaimStatusFromList(driver), "Open");
		...
		CPO_Add_Claim.claimAmount(driver).sendKeys("1000");
		test.log(LogStatus.PASS, " Claim Amount Added Successfully");
				
		wb.selectSingleValue(CPO_Add_Claim.selectClaimTypeFromList(driver), "General Liability");
		test.log(LogStatus.PASS, " Claim Type Added Successfully");

		CPO_Add_Claim.reportDate(driver).sendKeys("05/06/2020");
		Thread.sleep(2000);
		CPO_Add_Claim.reportDate(driver).sendKeys(Keys.TAB);
		test.log(LogStatus.PASS, " Claim Report Date Added Successfully");
		CPO_Add_Claim.CloseDate(driver).click();
		CPO_Add_Claim.CloseDate(driver).sendKeys("12/06/2020");
		Thread.sleep(2000);
		CPO_Add_Claim.CloseDate(driver).sendKeys(Keys.TAB);
		test.log(LogStatus.PASS, " Claim Close Date Added Successfully");
		CPO_Add_Claim.LossDate(driver).click();
		CPO_Add_Claim.LossDate(driver).sendKeys("05/06/2020");
		Thread.sleep(2000);
		CPO_Add_Claim.LossDate(driver).sendKeys(Keys.TAB);
		test.log(LogStatus.PASS, " Claim Loss Date Added Successfully");
		CPO_Add_Claim.LossTime(driver).click();
		CPO_Add_Claim.LossTime(driver).sendKeys("100");
		...
		js.executeScript("arguments[0].click();",CPO_Add_Claim.cliamantNameField(driver));
		CPO_Add_Claim.cliamantNameField(driver).sendKeys("Kiran");
		...
		CPO_Add_Claim.cliamantCity(driver).sendKeys("LA");
		...
		js.executeScript("arguments[0].click();",CPO_Add_Claim.cliamantLostDays(driver));
		CPO_Add_Claim.cliamantLostDays(driver).sendKeys("100");
		test.log(LogStatus.PASS, " Claimant Lost Days Added Successfully");
		js.executeScript("arguments[0].scrollIntoView();",CPO_Add_Claim.cliamantPartOfBody(driver));
		...
		js.executeScript("arguments[0].scrollIntoView();",CPO_Add_Claim.saveAndCloseBtn(driver));
