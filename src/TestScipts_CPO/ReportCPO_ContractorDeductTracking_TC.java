...
public class ReportCPO_ContractorDeductTracking_TC extends BaseClass
{
	...
	public void testContractorDeductTrackingReport(String ProgramName, String Division, String Projects) throws Throwable
	{		
		...
		try{
			...
			wb.selectSingleValue(Report_CPO_ContractorDeductTracking.filterSelectDivisionDropdown(driver), Division);
			test.log(LogStatus.PASS, Division+" Divisions Selected Successfully");
			Report_CPO_ContractorDeductTracking.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, Projects+" Project Selected Successfully");
			
			wb.selectSingleValue(Report_CPO_ContractorDeductTracking.filterSelectContractorFormOption(driver), "All Contractors");		
		...
		try {
			...
			Assert.assertEquals(Report_CPO_ContractorDeductTracking.reportHeaderReportName(driver).getText(), "Contractor Insurance Cost Tracking");
		...
		// Filter Testing
				...
				Thread.sleep(1000);
				js.executeScript("arguments[0].click();", Report_CPO_ContractorDeductTracking.btnBuildReport(driver));
				test.log(LogStatus.INFO, " Clicked On Build Report Button");
				Thread.sleep(3000);
...
Object[][] data = new Object[row-1][col];//store all row and column in multidimensional array and print one by one
	...
	public void testCount(int contPkgSize) {
		...
		List<WebElement> countOfContractorPkg = Report_CPO_ContractorDeductTracking.countProjectWiseContractorPkg(driver);
