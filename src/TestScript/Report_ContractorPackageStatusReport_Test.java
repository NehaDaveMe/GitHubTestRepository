...
public class Report_ContractorPackageStatusReport_Test extends BaseClass
{
	
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Contractor Package Status Report";
	String reportName="Contractor Package Status Report";
	
	@Test(dataProvider = "excelData" , invocationCount=3)
	public void testContractorPackageStatusReport(String ProgramName, String Division, String Projects,String userName, String password, String ProjectNAME1,String ProjectNAME2)
			throws Throwable {
		...
		try {
			...
			Thread.sleep(5000);
			Report_ContractorPackageStatusReport.selectReport(driver, selectReportName).click();
		...
		try {
			js.executeScript("arguments[0].scrollIntoView();",Report_ContractorPackageStatusReport.filterSelectDivisionDropdown(driver));
			...
			js.executeScript("arguments[0].scrollIntoView();", Report_ContractorPackageStatusReport.filterSelectContractorFormOption(driver));
		...
		try {
			...
			test.log(LogStatus.PASS, "Program Name:- " + Report_ContractorPackageStatusReport.programName(driver).getText());
			test.log(LogStatus.PASS, "Sponsor Name:- " + Report_ContractorPackageStatusReport.sponsorName(driver).getText());
		
			//test.log(LogStatus.PASS, "Project Name:- " + Report_ContractorPackageStatusReport.projectName(driver).getText());

			test.log(LogStatus.PASS, "Time Stamp:- " + Report_ContractorPackageStatusReport.timeStamp(driver).getText());
			test.log(LogStatus.PASS,
					"PDF Option is Present:- " + Report_ContractorPackageStatusReport.exportToPDF(driver).isDisplayed());
			test.log(LogStatus.PASS,
					"Excel Option is Present:- " + Report_ContractorPackageStatusReport.exportToExcel(driver).isDisplayed());
...
//		String filePath="D:\\NOURTEK\\Wrap Portal-2\\testData\\LocktonReportData2.xlsx";
	...
	public void testCount(int projectSize, int contPkgSize) {
		...
		List<WebElement> countOfProject = Report_ContractorPackageStatusReport.countProjectNames(driver);
		List<WebElement> countOfContractorPkg = Report_ContractorPackageStatusReport.countProjectWiseContractorPkg(driver);
