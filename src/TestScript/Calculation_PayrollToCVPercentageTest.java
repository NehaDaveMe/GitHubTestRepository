...
public class Calculation_PayrollToCVPercentageTest extends BaseClass {
	...
	@Test
	public void testPayrollToCVPercentage() throws InterruptedException {
...
//		AddContractor_Pkg_Insurance_Enrollment_Page.enrollContractorPkgAllTab(driver, tabName).click();
		...
		List<WebElement> listOfPremium = Report_PayrollToCVPercentage.takeAllEstimatedValue(driver);
		...
		try {
			...
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			// wb.selectSingleValue(Report_PayrollToCVPercentage.filterSelectDivisionDropdown(driver),
			// Division);
			// test.log(LogStatus.PASS, Division + " Selected Successfully");

			Report_PayrollToCVPercentage.filterSelectProjectFormOption(driver, projectName).click();
		...
		js.executeScript("arguments[0].scrollIntoView();",
				Report_PayrollToCVPercentage.selectContractorPkgID(driver, contractorPKGno));
		System.out.println(Report_PayrollToCVPercentage.selectContractorPkgID(driver, contractorPKGno).getText());

		List<WebElement> lst1 = Report_PayrollToCVPercentage.selectWCModifiersValue(driver, contractorPKGno);
